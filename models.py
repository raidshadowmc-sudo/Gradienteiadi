...
class Player(db.Model):
    ...
    def can_use_static_gradients(self):
        """Check if player can use static gradients (level 20+)"""
...
class PlayerQuest(db.Model):
    ...
    def check_completion(self, player_stat_value):
        ...
        self.current_progress = player_stat_value
        quest_obj = Quest.query.get(self.quest_id)
    ...
    def update_player_quest_progress(cls, player):
        """Update all quest progress for a player"""
        completed_quests = []
        
        # Get or create quest progress for all active quests
        active_quests = Quest.get_active_quests()
        
        for quest in active_quests:
            player_quest = cls.query.filter_by(
                player_id=player.id, 
                quest_id=quest.id
            ).first()
            ...
            if not player_quest:
                ...
                player_quest.quest_id = quest.id
                db.session.add(player_quest)
            
            # Update progress based on quest type
            stat_value = getattr(player, quest.type, 0)
            
            if player_quest.check_completion(stat_value):
                completed_quests.append(quest)
                
                # Award XP and title
                player.experience += quest.reward_xp
                if quest.reward_title and not player.role.endswith(quest.reward_title):
                    player.role = quest.reward_title
...
class Achievement(db.Model):
    ...
    def create_default_achievements(cls):
        ...
        default_achievements = [
            ...
            {
                'title': 'Неудержимый',
                'description': 'Убейте 5 игроков подряд без смерти',
                'icon': 'fas fa-fire',
                'rarity': 'rare',
                'unlock_condition': '{"streak_kills": 5}',
                'reward_xp': 250,
                'is_hidden': True
            },
            {
                'title': 'Коллекционер',
                'description': 'Соберите все виды ресурсов в одной игре',
                'icon': 'fas fa-coins',
                'rarity': 'epic',
                'unlock_condition': '{"collect_all_resources": True}',
                'reward_xp': 500,
                'is_hidden': True
            },
            {
